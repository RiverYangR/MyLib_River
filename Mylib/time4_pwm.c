...
/* ----------------------------------------------------------------------- 
  ...
  TIM_TimeBaseStructure.TIM_Period = 255;        //当定时器从0计数到2559，即为2560次，为一个定时周期
  ...
  TIM_OCInitStructure.TIM_Pulse = CCR_blue;	   //设置跳变值，当计数器计数到这个值时，电平发生跳变
  ...
  TIM_OCInitStructure.TIM_OutputState = TIM_OutputState_Enable;
  TIM_OCInitStructure.TIM_Pulse = CCR_green;	  //设置通道2的电平跳变值，输出另外一个占空比的PWM
